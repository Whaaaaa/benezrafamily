...
export async function initAviadDb() {
  ...
  await sql`ALTER TABLE aviad_assignments ALTER COLUMN class_id SET DEFAULT ''`
  await sql`ALTER TABLE aviad_assignments ADD COLUMN IF NOT EXISTS completed_at TEXT`
  await sql`
